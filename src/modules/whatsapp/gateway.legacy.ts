...
export function normalizeIncomingWhatsApp(
  ...
  // Vérification du provider supporté
  ...
  // Validation de la structure du payload 360dialog
  if (!payload.body || typeof payload.body !== 'object') {
    throw new WhatsAppNormalizationError(
      'Payload body manquant ou invalide',
      { body: payload.body }
    );
  }
  ...
  if (!webhookData.messages || !Array.isArray(webhookData.messages) || webhookData.messages.length === 0) {
    throw new WhatsAppNormalizationError(
      'Aucun message trouvé dans le payload 360dialog',
  ...
  if (!message.from) {
    throw new WhatsAppNormalizationError(
      'Champ "from" manquant dans le message',
      { message }
    );
  }
  ...
  if (!message.to) {
    throw new WhatsAppNormalizationError(
      'Champ "to" manquant dans le message',
      { message }
    );
  }
  ...
  if (!message.id) {
    throw new WhatsAppNormalizationError(
      'Champ "id" manquant dans le message',
      { message }
    );
  }
  ...
  if (!message.timestamp) {
    throw new WhatsAppNormalizationError(
      'Champ "timestamp" manquant dans le message',
      { message }
    );
  }

  // Extraction du texte (peut être vide pour les messages non-texte)
  let messageText = '';
  ...
  // Conversion du timestamp (Unix seconds → Date)
  let timestamp: Date;
  try {
